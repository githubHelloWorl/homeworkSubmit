<template>
	<div class="diy_details_box diy_div_learning_resource">
		<div class="warp diy_warp">
			...
			<div class="details_btn_wrap">
																<button class="btn btn-primary details_btn" v-if="$check_action('/learning_resource/details','set')" @click="change_praise()" ><span>点赞</span></button>
																<button class="btn btn-primary details_btn" v-if="$check_action('/learning_resource/details','set')" @click="change_collect()" ><span>收藏</span></button>
											</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "learning_resource",
					source_field: "learning_resource_id",
					source_id: this.obj["learning_resource_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/learning_resource/set?learning_resource_id=' + _this.obj["learning_resource_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
																		query.title = this.obj.resource_name
														query.img = this.obj.resource_images
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var learning_resource_id = this.obj.learning_resource_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "learning_resource",
          source_field: "learning_resource_id",
          source_id: learning_resource_id
        }, (res) => {
