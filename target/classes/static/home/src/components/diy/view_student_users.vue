		...
		<el-row class="row_ce">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="联系方式" prop="contact_information">
							  ...
							  v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','contact_information')) || (!form_sub['student_users_id'] && $check_field('add','contact_information'))"></el-input>
					<div v-else-if="$check_field('get','contact_information')">{{form_sub['contact_information']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','college_name') || $check_field('add','college_name') || $check_field('set','college_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="学院名称" prop="college_name">
								<el-select id="college_name" v-model="form_sub['college_name']"						v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','college_name')) || (!form_sub['student_users_id'] && $check_field('add','college_name'))">
					...
					<div v-else-if="$check_field('get','college_name')">{{form_sub['college_name']}}</div>
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','professional_name') || $check_field('add','professional_name') || $check_field('set','professional_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="专业名称" prop="professional_name">
								<el-select id="professional_name" v-model="form_sub['professional_name']"						v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','professional_name')) || (!form_sub['student_users_id'] && $check_field('add','professional_name'))">
						<el-option v-for="o in list_professional_name" :key="o['professional_name']" :label="o['professional_name']"
							:value="o['professional_name']">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','professional_name')">{{form_sub['professional_name']}}</div>
							</el-form-item>
			</el-col>
				...
				<el-form-item label="班级名称" prop="class_name">
								<el-select id="class_name" v-model="form_sub['class_name']"						v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','class_name')) || (!form_sub['student_users_id'] && $check_field('add','class_name'))">
	...
	export default {
		...
		methods: {
			...
			async get_list_professional_name() {
				var json = await this.$get("~/api/professional_information/get_list?");
				if(json.result && json.result.list){
					this.list_professional_name = json.result.list;
				}
				else if(json.error){
					console.error(json.error);
				}
