	...
	<el-main class="bg table_wrap comtabel_t">
			...
			<el-row class="rows row2">
				<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
					<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button
							v-if="user_group == '管理员' || user_group == '教师用户' || $check_action('/assignment_information/table', 'del') || $check_action('/assignment_information/view', 'del')"
		...
		<el-table :data="List" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%"
			...
			<el-table-column prop="teacher_users" @sort-change="$sortChange" label="教师用户"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_field('get', 'teacher_users')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_teacher_users(scope.row['teacher_users']) }}
				</template>
			</el-table-column>
			<el-table-column prop="teachers_name" @sort-change="$sortChange" label="教师姓名"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_field('get', 'teachers_name')" min-width="200">
			</el-table-column>
			<el-table-column prop="course_name" @sort-change="$sortChange" label="课程名称"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_field('get', 'course_name')" min-width="200">
			</el-table-column>
			<el-table-column prop="course_cover" @sort-change="$sortChange" label="课程封面"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_field('get', 'course_cover')" min-width="200">
				<template slot-scope="scope">
			...
			<el-table-column prop="assignment_title" @sort-change="$sortChange" label="作业标题"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_field('get', 'assignment_title')"
				min-width="200">
			</el-table-column>
			<el-table-column prop="assignment_content" @sort-change="$sortChange" label="作业内容"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_field('get', 'assignment_content')"
				min-width="200">
				<template slot-scope="scope">
					<a :href="$fullUrl(scope.row['assignment_content'])" target="_blank"
						style="color: rgb(64, 158, 255);">点击下载</a>
				</template>
			</el-table-column>
			<el-table-column prop="status" @sort-change="$sortChange" label="状态"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_field('get', 'status')" min-width="200">
				<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="300"
				v-if="user_group == '管理员' || user_group == '教师用户' || $check_action('/assignment_information/table', 'set') || $check_action('/assignment_information/view', 'set') || $check_action('/assignment_information/view', 'get') || $check_action('/提交/table', 'add') || $check_action('/提交/view', 'add')">
				<template slot-scope="scope">
					<div class="view_a btns">
						<el-button size="small" type="info">
							<router-link
								v-if="user_group == '管理员' || user_group == '教师用户' || $check_action('/assignment_information/table', 'set') || $check_action('/assignment_information/view', 'set') || $check_action('/assignment_information/view', 'get')"
								:to="'./view?' + field + '=' + scope.row[field]" size="small">
								<span>详情</span>
							</router-link>
						</el-button>
						<div style="margin-top: 5px;"><el-button size="small" @click="handlePublic(scope.row[field])"
								type="primary">发布</el-button></div>
						<el-button size="small" type="success">
							<router-link
								v-if="user_group == '管理员' || user_group == '教师用户' || $check_comment('/assignment_information/table')"
		...
		<div class="mt text_center">
			<el-pagination @size-change="get_assignment_information" @current-change="handleCurrentChange"
...
import { getList, toPublic } from "@/api/assignment_information/api.js"
...
export default {
	...
	methods: {
		...
		async get_assignment_information() {
			let user = JSON.parse($.db.get("user_group"));
			console.log(user);
			await getList(this.query).then((res) => {
				console.log(res);
				if (res.status === 200) {
					res.data.forEach((result) => {
						if (user.user_group == '教师用户' && user.nickname == result.teachers_name) {
			...
			await toPublic(id).then((res) => {
				if (res.status === 200) {
					...
					this.List.forEach((result) => {
						if (result.assignment_information_id == id) {
							result.status = 1;
						}
					})
				}
