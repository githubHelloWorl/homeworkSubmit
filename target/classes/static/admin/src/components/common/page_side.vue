<template>
  <el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
    ...
    <el-submenu index="user"
      ...
      <el-menu-item index="/teacher_users/table" v-show="user_group == '管理员' || $check_action('/teacher_users/table')"
        @click="handleButtonClick(8)">
        <span>教师用户</span></el-menu-item>
      <el-menu-item index="/student_users/table" v-show="user_group == '管理员' || $check_action('/student_users/table')"
        @click="handleButtonClick(8)">
    ...
    <el-submenu index="notic" v-show="user_group == '管理员' || $check_group(['/notice/table'])">
...
export default {
  ...
  computed: {
    auth() {
      ...
      var lt = [];
      let site_list = []
      site_list.push("homework_submission")
      site_list.push("homework_correction")
