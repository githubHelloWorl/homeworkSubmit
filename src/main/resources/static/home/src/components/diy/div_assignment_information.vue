<template>
	<div class="diy_details_box diy_div_assignment_information">
		<div class="warp diy_warp">
			<div class="container">
				<!-- <div class="status" v-if="obj.status == 1">已提交</div> -->
				...
				<div class="row row_det">
					<div class="row_top_wrap">
						...
						<div class="row_2" :class="{ flex_row2: !imgList.length }">
							<div class="row_cont" :class="{ flex_row2_item: !imgList.length }"
								v-for="(item, index) in itemList" :key="item + index"
								>
								<div class="view">
									<div class="diy_title">
										<span>{{ item.title }}:</span>
									</div>
									<div class="diy_field diy_uid" :class="{ flex_row2_item_text: imgList.length }"
										v-if="item.type == 'UID'">
										<span>
											{{ get_user_info(item.name, obj[item.name]) }}
										</span>
									</div>
									<div class="diy_field diy_video" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '视频'">
										<router-link :to="'/media/video?filename=' + $fullUrl(obj[item.name])"
											v-if="obj[item.name]">
											<span>
												查看视频
											</span>
									...
									<div class="diy_field diy_music" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '音频'">
										<audio v-if="obj[item.name]" style="text-align: left"
											:src="$fullUrl(obj[item.name])" controls></audio>
									</div>
									<div class="diy_field diy_music" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '文件'">
										<a :href="$fullUrl(obj[item.name])" target="_blank"
											style="color: rgb(64, 158, 255);">点击下载</a>
									</div>
									<div class="diy_field diy_date" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '日期'">
										<span>
											{{ $toTime(obj[item.name], "yyyy-MM-dd") }}
										</span>
									</div>
									<div class="diy_field diy_time" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '时间'">
										<span>
											{{ $toTime(obj[item.name], "hh:mm:ss") }}
										</span>
									</div>
									<div class="diy_field diy_datetime" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '日长'">
										<span>
											{{ $toTime(obj[item.name], "yyyy-MM-dd hh:mm:ss") }}
										</span>
									</div>
									<div class="diy_field diy_phone" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '电话' || item.type == '手机'">
										<span>
											{{ obj[item.name] }}
										</span>
									</div>
									<div class="diy_field diy_number" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '数字'">
										<span>
											{{ obj[item.name] }}
										</span>
									</div>
									<div class="diy_field diy_url" :class="{ flex_row2_item_text: imgList.length }"
										v-else-if="item.type == '网址'">
										<div>
											<a :href="obj[item.name]" target="_blank" style="color: rgb(64, 158, 255);">{{
												obj[item.name] }}</a>
										</div>
									</div>
									<div class="diy_field diy_desc" :class="{ flex_row2_item_text: imgList.length }"
					...
					<div class="details_btn_wrap">
						<button class="btn btn-primary details_btn" v-if="$check_action('/homework_submission/edit', 'add')"
							@click="to_form('/homework_submission/edit')"><span> 提交</span> </button>
						<button class="btn btn-primary details_btn"
							v-if="$check_action('/assignment_information/details', 'set')"
							@click="change_praise()"><span>点赞</span></button>
					</div>
					<!-- 富文本 -->
					<div class="rich_text" v-for="(item, index) in richList" :key="item + index"
...
export default {
	 ...
	 */
		...
		get_user_info(name, id) {
			var obj = null;
			if (name == 'teacher_users') {
				obj = this.list_user_teacher_users.getObj({ "user_id": id });
			}
			var ret = "";
			if (obj) {
	 ...
	 */
		change_praise(obj) {
			...
			var query = {
				source_table: "assignment_information",
				source_field: "assignment_information_id",
				source_id: this.obj["assignment_information_id"],
				user_id
			};
			...
			if (this.state_praise) {
				...
				this.$get('~/api/praise/del?', query, (res) => {
					if (res.result) {
						...
						this.$post('~/api/assignment_information/set?assignment_information_id=' + _this.obj["assignment_information_id"], {
							praise_len
						}, (res) => {
							if (res.result) {
	...
	created() {
		...
		console.log(this.obj['course_name']);
	},
	watch: {
		obj: {
			deep: true,
...
.status {
	...
	background-color: rgb(216, 243, 203);
