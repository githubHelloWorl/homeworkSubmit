<template>
	<div class="diy_details page_learning_resource" id="learning_resource_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_learning_resource'>
							<div_learning_resource :obj="obj"></div_learning_resource>
...
<script>
	import div_learning_resource from "@/components/diy/div_learning_resource.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/learning_resource/set?learning_resource_id=' + obj["learning_resource_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.learning_resource_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "learning_resource",
					source_field: "learning_resource_id",
					source_id: obj["learning_resource_id"],
					user_id
				};
