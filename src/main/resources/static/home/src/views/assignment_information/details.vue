<template>
	<div class="diy_details page_assignment_information" id="assignment_information_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_assignment_information'>
							<div_assignment_information :obj="obj"></div_assignment_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_assignment_information from "@/components/diy/div_assignment_information.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/assignment_information/set?assignment_information_id=' + obj["assignment_information_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.assignment_information_id;
				}
			},
			/**
			 * 获取点赞
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "assignment_information",
					source_field: "assignment_information_id",
					source_id: obj["assignment_information_id"]
				}, (res) => {
