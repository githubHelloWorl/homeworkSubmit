	...
	<el-main class="bg table_wrap comtabel_t">
			...
			<el-row class="rows row2">
				...
				<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
					...
					<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button
							v-if="user_group != '学生用户' || $check_action('/homework_submission/table', 'del') || $check_action('/homework_submission/view', 'del')"
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%"
			...
			<el-table-column prop="student_users" @sort-change="$sortChange" label="学生用户"
				v-if="user_group != '学生用户' || $check_field('get', 'student_users')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_student_users(scope.row['student_users']) }}
				</template>
			</el-table-column>
			<el-table-column prop="student_name" @sort-change="$sortChange" label="学生姓名"
				v-if="user_group != '学生用户' || $check_field('get', 'student_name')" min-width="200">
			</el-table-column>
			<el-table-column prop="teacher_users" @sort-change="$sortChange" label="教师用户"
				v-if="user_group != '学生用户' || $check_field('get', 'teacher_users')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_teacher_users(scope.row['teacher_users']) }}
				</template>
			</el-table-column>
			<el-table-column prop="teachers_name" @sort-change="$sortChange" label="教师姓名"
				v-if="user_group != '学生用户' || $check_field('get', 'teachers_name')" min-width="200">
			</el-table-column>
			<el-table-column prop="course_name" @sort-change="$sortChange" label="课程名称" min-width="200">
			</el-table-column>
			<el-table-column prop="assignment_title" @sort-change="$sortChange" label="作业标题"
				v-if="user_group != '学生用户' || $check_field('get', 'assignment_title')" min-width="200">
			</el-table-column>
			<el-table-column prop="submission_content" @sort-change="$sortChange" label="提交内容"
				v-if="user_group != '学生用户' || $check_field('get', 'submission_content')" min-width="200">
				<template slot-scope="scope">
					<a :href="$fullUrl(scope.row['submission_content'])" target="_blank"
			...
			<el-table-column fixed="right" label="操作" min-width="200"
				v-if="user_group != '学生用户' || $check_action('/homework_submission/table', 'set') || $check_action('/homework_submission/view', 'set') || $check_action('/homework_submission/view', 'get') || $check_action('/批改/table', 'add') || $check_action('/批改/view', 'add')">
				<template slot-scope="scope">
					<div class="view_a">
						<router-link class="e-button el-button--small is-plain el-button--success"
							style="margin: 5px !important;"
							v-if="user_group != '学生用户' || $check_action('/homework_submission/table', 'set') || $check_action('/homework_submission/view', 'set') || $check_action('/homework_submission/view', 'get')"
							:to="'./view?' + field + '=' + scope.row[field]" size="small">
							<span>详情</span>
						</router-link>
						<!--跨表按钮-->
						<el-button class="e-button el-button--small is-plain el-button--default"
							style="margin: 5px !important;" size="small" @click="
								to_table(scope.row, '/homework_correction/view');
								handleCorrection(scope.row)
								" v-if="user_group != '学生用户' || $check_action('/homework_correction/table', 'add') || $check_action('/homework_correction/view', 'add')">
							<span>批改</span>
						</el-button>
					</div>
				</template>
...
export default {
	...
	data() {
		return {
			...
			showModal: false,
			// 获取数据地址
			url_get_list: "~/api/homework_submission/get_list?like=0",
			url_del: "~/api/homework_submission/del?",
...
*/
		...
		handleCorrection(obj){
			Cookies.set('ass',JSON.stringify(obj))
		}
	},
	created() {
